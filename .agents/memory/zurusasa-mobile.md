...
- E2E testing gotcha: the Expo app lives on its own dev domain (`$REPLIT_EXPO_DEV_DOMAIN`), NOT the shared proxy. Give the testing agent the literal URL plus identity markers (chat titled "Zuru Agent", tabs ZuruFlow/Discover/Profile) — a bare "path /" sends it to the ZuruSasa WEB app, which intentionally looks near-identical (its chat is "Ask Zuru", nav "Pulse/Discover/Log In").
