...
export const DiscoverContent = ({
  ...
  // Filter Reels by city if selected
  ...
  const filteredEvents = useMemo(() => {
    if (!selectedCity || selectedCity === "Current Location") return events;
    return events.filter(event => event.location.toLowerCase().includes(selectedCity.toLowerCase()));
  }, [events, selectedCity]);
      ...
      try {
        ...
        if (data) {
          const transformed: ReelData = {
            ...
            // @ts-ignore - Supabase SelectQueryError union type; runtime shape is correct
