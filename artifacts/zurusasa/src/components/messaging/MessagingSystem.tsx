    ...
    useEffect(() => {
        ...
        const fetchConversations = async () => {
            ...
            const enriched = await Promise.all(data.map(async (conv: any) => {
                ...
                // @ts-ignore - Supabase typed query mismatch
                ...
                return {
                    ...conv,
                    other_participant: {
                        id: (profile as any)?.id || otherId,
                        full_name: (profile as any)?.full_name || "Zuru User",
                        username: (profile as any)?.username || "user",
                        role: (profile as any)?.role || "guest",
                        avatar_url: (profile as any)?.metadata?.avatar_url
                    }
                };
            }));
