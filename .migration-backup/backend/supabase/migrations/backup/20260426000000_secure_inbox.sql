...
BEGIN
    ...
    IF NEW.content ~ contact_pattern THEN
        NEW.is_flagged := TRUE;
        -- Optional: We can mask the content here, but it's better to warn the user or show a banner
        -- NEW.content := regexp_replace(NEW.content, contact_pattern, '[Contact Info Hidden for Security]', 'g');
    END IF;
